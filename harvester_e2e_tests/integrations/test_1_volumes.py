...
def test_create_volume(api_client, unique_name, ubuntu_image, create_as, source_type, polling_for):
    ...
    if source_type == "VM Image":
        assert image_id == mdata['annotations']['harvesterhci.io/imageId'], (code, data)
    else:
        assert not mdata['annotations'].get('harvesterhci.io/imageId'), (code, data)
    ...
    assert not mdata['annotations'].get("harvesterhci.io/owned-by"), (code, data)
...
class TestVolumeWithVM:
    ...
    def test_delete_volume_bounded_on_existing_vm(self, api_client, ubuntu_vm, polling_for):
        """
    ...
    def test_delete_volume_bounded_on_deleted_vm(self, api_client, ubuntu_vm, polling_for):
        """
        1. Create a VM with volume
        2. Delete volume should reply 422
