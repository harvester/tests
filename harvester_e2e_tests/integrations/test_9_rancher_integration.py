...
@pytest.fixture(scope="class")
def ip_pool(api_client, unique_name, vlan_network):
    code, data = api_client.ippools.create(unique_name, vlan_network["id"])
    assert 201 == code, (
        f"Failed to create ip pool {unique_name} with error {code}, {data}"
    )

    yield

    api_client.ippools.delete(unique_name)


@pytest.fixture(scope='class', params=["dhcp", "pool"])
def lb_service(request, unique_name, nginx_deployment, ip_pool):
    namespace = "default"
    name = f"lb-{request.param}-{unique_name}"
    data = {
        "type": "service",
        "metadata": {
            "namespace": namespace,
