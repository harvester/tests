...
def upgrade_target(request):
    ...
    checksum = request.config.getoption("--upgrade-target-version")
...
def stopped_vm(request, api_client, ssh_keypair, wait_timeout, unique_name, image):
    unique_vm_name = f"{request.node.name.lstrip('test_')}-{unique_name}"
    cpu, mem = 1, 2
...
class TestInvalidUpgrade:
    def test_degraded_volume(
        ...
        assert code == 204, f"Failed to delete version {version}: {data}"

    def test_invalid_manifest(self, api_client, unique_name, wait_timeout):
        """
        Steps:
        ...
        assert version == data['spec']['version'], (code, data)
        ...
        # teardown
        code, data = api_client.upgrades.delete(data['metadata']['name'])
