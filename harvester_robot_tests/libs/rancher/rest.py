...
class Rest(Base):
    ...
    def install_rbac_chart(self, chart_repo_url, chart_version, release_name, namespace):
        ...
        Adds the chart repository, then runs helm install against Rancher's local
        cluster using a dynamically generated kubeconfig.  Idempotent: skips if
        ...
        # Add + update the repo (no kubeconfig needed for repo operations)
        add_result = subprocess.run(  # nosec B603
            ["helm", "repo", "add", repo_alias, chart_repo_url, "--force-update"],
            capture_output=True, text=True
        )
        if add_result.returncode != 0:
            raise Exception(
                f"Failed to add Helm repo '{chart_repo_url}': {add_result.stderr}"
            )
        ...
        # Build a temporary kubeconfig targeting Rancher's local cluster
        self._authenticate_rancher()
        k8s_endpoint = f"{self.rancher_endpoint.rstrip('/')}/k8s/clusters/local"
        kubeconfig_data = {
            "apiVersion": "v1",
            "kind": "Config",
            "clusters": [{"name": "rancher-local", "cluster": {
                "server": k8s_endpoint,
                "insecure-skip-tls-verify": True
            }}],
            "contexts": [{"name": "rancher-local", "context": {
                "cluster": "rancher-local", "user": "rancher-admin"
            }}],
            "current-context": "rancher-local",
            "users": [{"name": "rancher-admin", "user": {
                "token": self.rancher_token
            }}]
        }

        fd, kubeconfig_path = _tempfile.mkstemp(
            suffix=".kubeconfig", prefix="rancher-rest-"
        )
        try:
            ...
            if result.returncode != 0:
                if "already exists" in result.stderr or \
                        "cannot re-use" in result.stderr:
                    logging(
                        f"Helm release '{release_name}' already exists; skipping"
                    )
                else:
                    raise Exception(
                        f"Failed to install harvester-rbac chart "
                        f"v{chart_version}: {result.stderr}"
                    )
            else:
                logging(
                    f"Installed harvester-rbac chart v{chart_version} "
                    f"as '{release_name}'"
                )
        finally:
