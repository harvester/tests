...
Initial Nvidia Addon State Is Captured
    ...
    ${initial_state}=    addon.Get Addon Initial State    ${addon_name}
...
Nvidia Addon Is Enabled
    ...
    ${is_enabled}=    addon.Is Addon Enabled    ${addon_name}
    Run Keyword If    not ${is_enabled}    addon.Enable Addon    ${addon_name}
    addon.Wait For Addon Enabled    ${addon_name}    timeout=900
...
Suite Teardown For Nvidia Addon Tests
    ...
    ...    addon.Restore Addon State    ${ADDON_NVIDIA_TOOLKIT}    ${INITIAL_STATE_NVIDIA_TOOLKIT}
...
PCI Devices Controller Pods Should Be Running
    ...
    addon.Wait For Addon Pods Running    ${namespace}    ${label}    timeout=900
...
PCI Devices Webhook Service Should Be Running
    ...
    addon.Wait For Addon Service Running    ${namespace}    ${service_name}    timeout=300
